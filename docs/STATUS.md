...
| **19** | **The NL→Cedar compiler scores 0/30 on its own dataset.** First real measurement, now the harness works: **passed 0 · wrong decision 4 · unparseable Cedar 2 · asked for clarification 23 · timeout 1**. Latency median 25.9 s, max 300.6 s. The dominant failure is not bad Cedar — it is the compiler **refusing to answer**, asking for clarification on prompts as plain as *"Managers can approve expenses."* The ambiguity instruction in `_SYSTEM_PROMPT` over-fires on 77% of the corpus | ADR-033 and this page previously said T-029 was "evaluated against a 30-case corpus". It never was. Demo beat 5 is budgeted 90 s end-to-end and the model alone takes 25.9 s median; with 0% accuracy and T-031 (fallback) deferred, **beat 5 is not currently demonstrable** | Decide between: rebalance the ambiguity instruction (cheapest, targets 23 of the 30), un-defer T-031, or rescope the beat. Needs a decision, not a default |

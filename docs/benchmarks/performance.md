...
Budget: **p99 < 1 ms**. Measured 2026-09-06T22:48:01.837187+00:00.
...
|---|---|---|
| step 1 — extract (route, args, digest) | 19.8 | 31.6 |
| step 2 — verify (biscuit signature) | 204.9 | 381.0 |
| step 4 — caveats (4 clauses, Datalog) | 3.6 | 7.7 |
| step 4b — token authority (biscuit authorizer, depth 1) | 102.1 | 192.4 |
| step 5 — policy (Cedar) | 131.5 | 230.8 |
| step 10 — audit record hash | 15.7 | 25.5 |
| **`decide()` total — NFR-1** | 264.1 | 395.1 |

**NFR-1 holds**: `decide()` p99 is 395.1 µs against a 1000 µs budget, and `PLAN.md` §17's R-2 trigger (2 ms, port to Rust) is not approached.

Three things the breakdown says that a single number cannot:

- **Two steps are the decision.** Cedar at 131.5 µs and the token's own Datalog at 102.1 µs together account for almost all of a 264.1 µs median. Everything else inside `decide()` is single-digit microseconds.

- **Enforcing the token costs about as much as the policy does, and buys more.** Step 4b evaluates every check in every block — the mandate's grant and every attenuation caveat — against a token that may have arrived from anywhere. It was absent until the manual test pass found that nothing on the PEP path called `authorize()`, so the 102.1 µs is new cost against a budget that still has room, not a regression.

- **`verify()` is the most expensive step and sits *outside* `decide()`** at 204.9 µs median. Per-request in-process cost is verify + decide + extract, so roughly 489 µs — that, not NFR-1 alone, is what bounds throughput.

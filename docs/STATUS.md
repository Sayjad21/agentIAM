...
**Last updated:** after T-054 (security scanning + SBOM).
...
| | |
|---|---|
| **Milestone** | **M1–M4 complete** · M5 started (T-024, T-025, T-026, T-027 done) · M6 (T-028, T-029, T-030 done) · M7 started (T-032, T-033, T-036 done) · **M8 complete (T-037…T-040 done)** · **M9 resolved (T-043 done; T-041/T-042/T-044 deferred — nothing left undeferred)** · **M10 complete (T-045…T-050 done)** · **M11 under way (T-051, T-052, T-053, T-054 done)** |
| **Tickets** | 44 done · 8 remaining · 9 deferred · **61 defined, 52 in scope** |
| **Tests** | 2275 passing (2017 in `make test`; 235 in `make test-integration`; 13 in `make test-e2e`; 10 in `make bench`; 10 in `make chaos`) |
...
| **CI** | green — seven jobs: lint/types/tests + **NFR-1 benchmark**, integration against real Postgres, **the end-to-end slice**, **chaos (nightly cron only)**, core purity, compose health, **security-scan (bandit + pip-audit + trivy + gitleaks + SBOM check + log secret-scanner)** |
...
|---|---|---|
| T-055…T-059 | Evidence pack generator, deployment artifacts, submission package, demo drills | M11/M12 |
...
| 21 | **A partitioned PEP cannot shut down, and a timeout does not bound it.** `LeasePool.aclose()` drains in-flight top-ups and then `RELEASE`s, both of which need the ledger. Worse, `asyncio.wait_for` around it does not help: the cancellation lands inside SQLAlchemy's greenlet bridge while asyncpg is blocked on the dead socket, and the driver's own rollback/close needs that socket — measured stuck for 5 minutes against a 5 s bound | Availability on restart, not correctness: the stranded lease is bounded by TTL + S (CH-3) and `REAP` reclaims it. But an orchestrator's `SIGTERM` grace period elapses and the PEP is killed anyway, turning a graceful shutdown into CH-3 | Pinned by CH-4's `test_a_partitioned_pep_cannot_shut_down_gracefully`. A fix needs a release path that can abandon its connection |

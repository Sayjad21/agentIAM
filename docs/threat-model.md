...
> the twenty-seven entries here are not fully mitigated, and each says why.
...
| TM-27 | E | **A token's authority check that no code path evaluates.** The authority block carries six checks (spec 01 §5) and biscuit's authorizer enforces all six — but `verify()` deliberately never calls `authorize()`; it extracts facts, so that reason codes can be precise. Every check `decision.decide()` does not *re-implement* is therefore enforced nowhere on the path the PEP actually runs. Measured while wiring T-023: a request whose `request_intent` did not match the token's was **allowed** by `decide()` while biscuit denied the identical request. That is INV-7's binding and TM-10's defence against task redirection, both decorative on the live path. Five of the six were covered — scope by `decide()`, the validity window and the chain's depth by `verify()`, the mandate's budget ceiling by the ledger that issues the lease — and intent by nothing | `decide()` step 4 now checks the request's intent against the token's, and the verifier-supplied `current_depth` against the mandate's `max_depth`. Ordered before the caveat loop, so a wrong task is reported as `INTENT_MISMATCH` rather than as whichever caveat happens to fail (spec 09 §3) | mitigated | `test_decision.py::TestTokenAuthorityIsActuallyEnforced` — four cases, including that intent outranks a failing caveat |
...
TM-19 through TM-22 and TM-24 through TM-27 should be counted as additions to that set, not substitutions:
...
| Elevation of privilege | TM-02, TM-03, TM-04, TM-05, TM-10, TM-11, TM-19, TM-20, TM-26, TM-27 | 8 of 10 |

**27 threats · 20 mitigated · 4 partially mitigated · 3 accepted risks.**
...
each has a bound, and stating them is what makes the other twenty-four believable.

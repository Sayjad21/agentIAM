...
| F-2 | Ollama slow or down | Beat 5 (NL→Cedar) only. **T-031's template fallback does not exist** — it is deferred (`ROADMAP.md`, `STATUS.md` line 112), and `tests/chaos/test_ch08_ollama_down.py` asserts its absence. Do not narrate a failsafe. | The compiler surfaces the failure and the console stays up: measured against the demo stack with Ollama unreachable, `POST /policy/compile` returns promptly with *"Ollama network error: All connection attempts failed"* in the results panel. Narrate what is true: "the compiler is the one hosted dependency, it failed closed, and nothing else in the system noticed — enforcement never left this machine." Then move to beat 6. |

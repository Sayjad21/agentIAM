...
def test_score_is_high_for_orthogonal_intent(
    mock_post: MagicMock, drift_oracle: RuleBasedDriftOracle
) -> None:
    context = RequestContext.model_construct(
        task_intent_text="Transfer 500 units to vendor A",
...
@patch("agentiam_pep.drift.httpx.Client.post")
def test_score_scales_linearly_with_similarity(
    mock_post: MagicMock, drift_oracle: RuleBasedDriftOracle
) -> None:
    context = RequestContext.model_construct(
        task_intent_text="Transfer 500 units to vendor A",
...
@patch("agentiam_pep.drift.httpx.Client.post")
def test_fails_open_if_oracle_is_down(
    mock_post: MagicMock, drift_oracle: RuleBasedDriftOracle
) -> None:
    context = RequestContext.model_construct(
        task_intent_text="Transfer 500 units to vendor A",
...
@patch("agentiam_pep.drift.httpx.Client.post")
def test_fails_open_if_oracle_returns_error(
    mock_post: MagicMock, drift_oracle: RuleBasedDriftOracle
) -> None:
    context = RequestContext.model_construct(
        task_intent_text="Transfer 500 units to vendor A",

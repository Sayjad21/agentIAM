...
| **Tests** | 2242 passing (1985 in `make test`; 230 in `make test-integration`; 13 in `make test-e2e`; 4 in `make bench`; **10 in `make chaos`**, all five scenarios green in 3m05s) |
...
| 20 | **A transient audit-sink outage is treated as a poison batch.** `DecisionEmitter._write_batch_locked` gives a batch `max_retries` attempts and then **discards** it. That path was written for one record the sink will never accept, but a stopped database is indistinguishable from it at that layer, so batches are dropped roughly every `(max_retries + 1) x flush_interval_s`, the queue never fills, and ADR-026's deny-on-full back-pressure never engages | The PEP keeps authorizing requests it can no longer record — the exact outcome ADR-026 exists to prevent. The loss is *counted* (`lost_records`), never silent, which is the only guarantee that actually holds | Measured by CH-1's `test_the_audit_path_is_the_weak_link`. The fix is the one ADR-049 already applies to settlement: distinguish "sink unreachable" (retry) from "sink refuses this record" (give up) |
